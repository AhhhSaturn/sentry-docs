...
`@sentry/effect` supports **Effect v3** and **Effect v4**. The integration detects the installed Effect version at runtime, but the `Tracer` and `Logger` layer APIs differ between major versions, so use the **Effect v3** or **Effect v4** snippets below for your app.
...
The SDK provides an `effectLayer` that initializes Sentry. You can compose it with additional Effect layers to enable tracing, logging, and metrics. The `effectLayer`, `SentryEffectTracer`, `SentryEffectLogger`, and `SentryEffectMetricsLayer` exports are the same for Effect v3 and v4; only the layer composition around tracing and logging changes.

### Effect v3

<AvailableSince version="10.44.0" />

On **Node**, provide `SentryLive` before launching your HTTP layer (for example with `NodeRuntime`). In the **browser**, provide `SentryLive` to your app layer. Use `Layer.setTracer` and `Logger.replace` for tracing and logs.
