...
The temptation is to add monitors for everything. The better approach is to start with jobs and endpoints where a silent failure has a direct, delayed impact — problems you'd otherwise only discover when a customer complains.

### Cron jobs worth monitoring

Not every background job needs a monitor. Focus on jobs where a missed or failed run causes visible business impact:
...
- **User-facing delivery**: Email sends, push notification delivery, digest generation
- **Compliance or audit**: Log archival, GDPR deletion jobs, report generation

Lower priority: maintenance cleanup jobs, cache warming, analytics aggregation. Add monitors for these once the high-priority ones are covered.

### Endpoints worth uptime monitoring

Monitor endpoints at the level your users experience, not just your internal health check:
...
- Clock skew between your job scheduler and Sentry's system
...
If your job uses the Sentry SDK, [link errors to the monitor](/product/crons/getting-started/http/#connecting-errors-to-cron-monitors) by setting the monitor slug in your SDK initialization for that job. This surfaces any errors thrown during the job run directly on the monitor's detail page, alongside the check-in timeline — instead of only in Issues.

## Configuring Uptime Monitors Well

### Start with automatic detection

Sentry [automatically configures an uptime monitor](/product/new-monitors-and-alerts/monitors/uptime-monitoring/automatic-detection/) for the most frequently seen hostname in your error data. Check this first — you may already have uptime monitoring for your primary domain without setting anything up.

### Failure threshold

By default, Sentry requires **three consecutive failures** before creating an uptime issue. This prevents false positives from transient network issues or one-off timeouts. You can adjust this threshold when configuring the monitor, but the default is appropriate for most teams.
